...
(float)(Math.Sqrt(Vector3.Dot(vec2, vec2)) * vec1.Z));
        ...
        public static void setInOutM(List<Strand> strandlist, string outputDirectory, string pdbName, Vector3 CCentroid, Vector3 NCentroid) //10-07-20 - RD - Use the reference point on axis
        {
            Dictionary<Vector3, Vector3> nearestPoint = new Dictionary<Vector3, Vector3>();
            //string fileLocation3 = outputDirectory + "InOut\\InOut_" + pdbName + ".txt";
            //using (System.IO.StreamWriter file = new System.IO.StreamWriter(fileLocation3))
            ...
            //SharedFunctions.writePymolScriptForNearestPoint(Global.MONO_OUTPUT_DIR, Global.MONO_DB_DIR, pdbName, nearestPoint);
            //SharedFunctions.WritePymolScriptForInOutStrands(strandlist, Global.MONO_OUTPUT_DIR, Global.MONO_DB_DIR, pdbName);
        }
